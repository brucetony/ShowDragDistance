...
class DragRuler extends Ruler {
  ...
  getTokenSpeed(token) {
    ...
    Object.assign(
      flaggedSpeed,
      token.document.getFlag("ShowDragDistance", "speed") || {}
    );
    const normalSpeed = baseSpeed + flaggedSpeed.normal;
    const dashSpeed =
      (normalSpeed + flaggedSpeed.dash) *
      game.settings.get("ShowDragDistance", "dashX");
  ...
  _highlightMeasurement(line) {
    ...
    for (let i = 0; i < line.length; i++) {
      if (line[i].travelled > remainingSpeed) {
        if (
          game.settings.get("ShowDragDistance", "dash") &&
          line[i].travelled < remainingSpeed + maxSpeed
        ) {
          color = colorStringToHex(
            game.settings.get("ShowDragDistance", "dashSpeedColor")
          );
        } else if (
          game.settings.get("ShowDragDistance", "dash") == false ||
          line[i].travelled > remainingSpeed + maxSpeed
        ) {
          color = colorStringToHex(
            game.settings.get("ShowDragDistance", "maxSpeedColor")
          );
        }
      }
  ...
  static init() {
    ...
    TokenSpeedAttributes.bonus =
      game.settings.get("ShowDragDistance", "bonusSpeedAttr") !== ""
        ? game.settings.get("ShowDragDistance", "bonusSpeedAttr")
        : "";
    ...
    Token.prototype._onDragLeftStart = function (event) {
      if (
        game.settings.get("ShowDragDistance", "enabled") === true &&
    ...
    Token.prototype._onDragLeftDrop = function (event) {
      if (
        game.settings.get("ShowDragDistance", "enabled") &&
        canvas.controls.dragRuler.active
      ) {
    ...
    switch (e.which) {
      case 17:
        ...
        if (
          ...
          !rangeFinder &&
          canvas.tokens.controlled.length > 0 &&
          game.settings.get("ShowDragDistance", "rangeFinder") === true &&
  ...
  linedraw(b, totalDistance = 0, ignoreTerrain = false) {
    ...
    for (var i = 0; i <= N; i++) {
      ...
      if (i == 0) dist = 0;
        ...
        if (
          typeof canvas.terrain?.cost[grid.row]?.[grid.col] != "undefined" &&
          !ignoreTerrain
        ) {
